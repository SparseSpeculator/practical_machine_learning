...
## Analysis
As we see, the models perform quite differently on the test dataset. The best accuracy is achieved by the random forest.
```{r}
model_stats
```

This performance carries over to the test-set. The random forest is almost as accurate on fresh data. We expect the out-of sample error to be in the same ballpark for different data.

```{r, echo=FALSE}
confusionMatrix(dtest$class,factor(predict(m_rfo$finalModel, dtest)))
```
